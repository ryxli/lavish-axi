...
import { DEFAULT_TEMPLATE, contractsForEnvironment } from "./templates/contracts.js";

// Templates are resolved from bundled dist/templates/ at runtime.
...
export function listKnownTemplates() {
  const contracts = contractsForEnvironment();
  try {
    return Object.keys(contracts)
      .filter((template) => {
        try {
          return readdirSync(templatesDir()).includes(`${template}.html`);
        } catch {
          return false;
        }
      })
