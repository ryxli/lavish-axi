...
export function createHomeOutput({ bin, sessions, includeSessions = true }) {
  return {
    ...
    help: [
      "Fastest start: run `lavish-axi new [--template <name>] [output-path]` to scaffold a ready artifact (default `firstmate` template) with theme, overflow guards, and window.lavish interaction JS already baked in, then replace the placeholder content - prefer this over hand-writing boilerplate when no custom design is required",
