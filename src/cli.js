...
const COMMANDS = new Set(["open", "poll", "end", "stop", "server", "playbook", "design", "setup", "new"]);
const DESCRIPTION =
...
export function createHomeOutput({ bin, sessions, includeSessions = true }) {
  return {
    ...
    help: [
      "Fastest default loop: run `lavish-axi new [--template <name>] [output-path]` to scaffold the default `surface` template - a self-contained review surface with inline styling, overflow guards, and window.lavish interaction JS already baked in. Replace the placeholder content in `<main>`, then run `lavish-axi <html-file>` and `lavish-axi poll <html-file>`. Reach for `plan`, `comparison`, `report`, `decision`, or the legacy `firstmate` gallery only when that preset genuinely fits better",
...
const COMMAND_HELP = {
  ...
  new: `Usage: lavish-axi new [--template <name>] [output-path]\n\nScaffold a new HTML artifact from a built-in template (default template: surface). The template is written to .lavish/<name>.html in the current directory by default, or to the optional output-path when provided. The output directory is created automatically.\n\nThe default \`surface\` scaffold is a self-contained review surface with inline CSS, overflow guards, and window.lavish wiring already baked in, so the normal loop is: \`lavish-axi new\`, edit \`<main>\`, \`lavish-axi <file>\`, then leave \`lavish-axi poll <file>\` running. Other presets stay available when a narrower shape helps: \`plan\`, \`comparison\`, \`report\`, \`decision\`, plus the legacy \`firstmate\` gallery.\n\nSet LAVISH_AXI_TEMPLATES_DIR to scaffold from a custom template library instead.\n\nExamples:\n  lavish-axi new\n  lavish-axi new --template plan\n  lavish-axi new --template report docs/review.html\n`,
  ...
  design: `Usage: lavish-axi design\n\nShow a copy-pasteable CDN snippet for Tailwind CSS browser runtime v4 + DaisyUI v5 + themes, Mermaid diagram tooling, a content-to-playbook router, an optional layout safety CSS snippet, plus technical reference for DaisyUI components. ${PLAYBOOK_ROUTER_HELP} Lavish artifacts stay portable HTML. This command is fallback, not the default path: start with the self-contained \`surface\` scaffold, inspect the subject project before falling back, and paste the layout safety CSS only when useful for dense nested grid/flex layouts, badges, wide fonts, or local media. The strict priority order is: (1) if the user asked for a specific look or named design system, follow that; (2) otherwise, match the design system of the project the artifact is about, not necessarily your current working directory. If the artifact previews, proposes, or mocks a specific app's UI, use that app's own design system; (3) only when both come up empty, prefer the Lavish Tailwind CSS browser runtime v4 + DaisyUI v5 CDN snippet over hand-writing styles unless explicitly instructed otherwise by the user.\n`,
