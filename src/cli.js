...
export function createHomeOutput({ bin, sessions, includeSessions = true }) {
  return {
    ...
    help: [
      "Fastest start: run `lavish-axi new [--template <name>] [output-path]` to scaffold a ready artifact (default `firstmate` template) with the Lavish brand foundation, inline CSS plus a font import, overflow guards, and window.lavish interaction JS already baked in, then replace the placeholder content - prefer this over hand-writing boilerplate when no custom design is required",
...
const COMMAND_HELP = {
  ...
  new: `Usage: lavish-axi new [--template <name>] [output-path]\n\nScaffold a new HTML artifact from a built-in template (default template: firstmate). The template is written to .lavish/<name>.html in the current directory by default, or to the optional output-path when provided. The output directory is created automatically.\n\nAll templates share one Lavish brand foundation: inline CSS plus a font import, one brass accent, Garamond + Geist typography, overflow guards, and standalone rendering. Each preset leads with a treatment suited to the artifact type while still carrying the shared foundation. Every scaffolded file embeds all section CSS, so you can add, reorder, or delete the <!-- ==SECTION:name== --> blocks to fit the task.\n\nConcept presets:\n  firstmate    hybrid gallery of every section\n  plan         causal-diagram-led plan scaffold\n  comparison   operations-desk-led comparison scaffold\n  report       operations-desk-led report scaffold\n  decision     reading-room-led decision scaffold\n\nSet LAVISH_AXI_TEMPLATES_DIR to scaffold from a custom template library instead.\n\nExamples:\n  lavish-axi new\n  lavish-axi new --template plan\n  lavish-axi new --template report docs/review.html\n`,
