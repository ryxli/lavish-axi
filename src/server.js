...
export function createChromeHtml(
  ...
  const rawHeaderTitle = String(title || "")
    .replace(/\s+·\s+Lavish$/i, "")
    .trim();
  const headerTitle =
    rawHeaderTitle && rawHeaderTitle !== "Lavish Editor"
      ? rawHeaderTitle
      : path.basename(session.file, path.extname(session.file));
  const headerFile = path.basename(session.file);
  return `<!doctype html>
<html>
<head>
<meta charset="utf-8">
