...
export const DESIGN_SYSTEM_HINT =
  "Lavish does not auto-inject any design system - artifacts stay portable so they render identically when opened directly without lavish-axi running. The default `lavish-axi new` path is the self-contained `surface` scaffold: start there, replace the content in `<main>`, then open it and leave `lavish-axi poll` running. Before writing any HTML and before reaching for extra design tooling, decide the design direction in this strict priority order, and only move to the next step when the current one truly yields nothing: (1) if the user asked for a specific look or named design system, use that; (2) otherwise you must first inspect the project the artifact is about - the subject or product whose content or UI it represents, which may differ from your current working directory - and match that project's design system: Tailwind or theme config, shared CSS variables or design tokens, component library, brand assets, or existing styled pages. If the artifact previews, proposes, or mocks a specific app's UI, render it in that app's own design system so it faithfully shows the product, even when you are running in a different repo; (3) only when both steps come up empty, use `lavish-axi design` for the Lavish-recommended Tailwind CSS browser runtime v4 + DaisyUI v5 CDN snippet, a Mermaid CDN snippet/init for diagrams, and the DaisyUI component reference, and prefer that CDN snippet over hand-writing styles unless the user explicitly instructed otherwise. When you deliver the artifact, state which of the three design sources you used and why.";
...
export function createDesignOutput() {
  return {
    ...
    design: {
      summary:
        "Use this Lavish CDN fallback only if the default `surface` scaffold is not enough, the user gave no design direction, and you already inspected the project the artifact is about - the subject or product whose content or UI it represents, which may differ from your current working directory - and found no design system or style conventions to match. If you have not checked the subject project yet, check first. Lavish does not auto-inject any design system; artifacts stay portable HTML. The strict priority order is: (1) a look or named design system the user asked for; (2) the subject project's design system or style conventions - look for a Tailwind or theme config, shared CSS variables or design tokens, a component library, brand assets, or existing styled pages. If the artifact previews, proposes, or mocks a specific app's UI, use that app's own design system; (3) only when both come up empty, prefer this Tailwind CSS browser runtime v4 + DaisyUI v5 CDN snippet over hand-writing styles unless the user explicitly instructed otherwise.",
    ...
    theme_usage: [
      'If you need a quick DaisyUI fallback, `<html data-theme="luxury">` is the Lavish demo theme here, but it is only a fallback. Prefer the self-contained `surface` scaffold or the subject product\'s own look when either is available.',
