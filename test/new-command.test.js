...
test("newCommand writes the template file to default path", async () => {
  ...
  try {
    ...
    assert.match(written, /Canonical Lavish brand foundation/);
    assert.match(written, /window\.lavish/);
    assert.match(written, /data-treatment="hybrid-gallery"/);
...
test("newCommand defaults to the firstmate template when --template is omitted", async () => {
  ...
  try {
    ...
    assert.match(written, /<!doctype html>/i);
    assert.match(written, /Lavish hybrid gallery/);
  } finally {
    process.chdir(origCwd);
    await rm(dir, { recursive: true, force: true });
  }
...
test("firstmate template keeps the Lavish foundation inline without external stylesheet or script tags", async () => {
  const content = await readFile(fileURLToPath(new URL("../dist/templates/firstmate.html", import.meta.url)), "utf8");
  assert.doesNotMatch(content, /<link rel="stylesheet"/i, "no external stylesheet tags");
  assert.doesNotMatch(content, /<script src="https?:/i, "no external script tags");
  assert.match(content, /<style>/, "has inline style block");
  assert.match(content, /Canonical Lavish brand foundation/, "Lavish foundation stays inline");
});
...
test("firstmate template has the updated cheatsheet comment", async () => {
  const content = await readFile(fileURLToPath(new URL("../dist/templates/firstmate.html", import.meta.url)), "utf8");
  assert.match(content, /LAVISH BRAND FOUNDATION TEMPLATE/, "cheatsheet header");
  assert.match(content, /FOUNDATION:/, "foundation guidance");
  assert.match(content, /TREATMENTS BY PRESET:/, "preset treatments");
  assert.match(content, /INTERACTION PATTERNS:/, "interaction patterns");
});
...
test("firstmate template submitDecision keeps the recommended badge out of the queued decision label", async () => {
  const content = await readFile(fileURLToPath(new URL("../dist/templates/firstmate.html", import.meta.url)), "utf8");
  const submitDecisionSource = content.match(/function submitDecision\(\) \{[\s\S]*?\n      \}/)[0];
  let queuedPrompt = "";
  ...
  const selected = {
    ...
    closest() {
      return {
        textContent: "Option A recommended",
        querySelector(selector) {
          return { textContent: selector === ".option-title" ? "Option A" : "Option A recommended" };
        },
      };
    },
  };
  const submitDecision = new Function(
    "document",
    "window",
    `${submitDecisionSource}; return submitDecision;`,
  )(
    {
      querySelector(selector) {
        return selector === 'input[name="decision"]:checked' ? selected : null;
      },
      getElementById(id) {
        if (id === "sent-msg") return sent;
        if (id === "submit-decision") return button;
        return null;
      },
    },
    {
  ...
  submitDecision();
  assert.equal(queuedPrompt, "Decision: Option A (value: option_a)");
});
