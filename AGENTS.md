...
`lavish-axi new [--template <name>] [output-path]` (`src/new-command.js`) scaffolds an artifact from `src/templates/<name>.html`, defaulting to the `firstmate` template (`DEFAULT_TEMPLATE`) so template-first is the zero-friction path; templates are auto-discovered by reading `src/templates/` and `scripts/build.js` copies them to `dist/templates/`.
